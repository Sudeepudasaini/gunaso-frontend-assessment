...
// //   const [open, setOpen] = React.useState(false); // confirm modal
...
// //     e.preventDefault();
// //     setOpen(true); // "Are you sure?" dialog
// //   };
...
// //       router.push(`/submit/success/${data.id}`);
// //     } finally {
// //       setSaving(false);
// //       setOpen(false);
// //     }
...
// //       <form onSubmit={handleSubmit} className="grid gap-4" aria-describedby="submit-help">
// //         {/* Category */}
// //         <div>
// //           <Select
// //             id="category"
...
// //           >
// //             {categories.map(c => (
// //               <SelectItem text={c} value={c} key={c} />
// //             ))}
// //           </Select>
// //         </div>
...
// //             onChange={(e: any) => setForm(f => ({ ...f, district: e.target.value }))} />
...
// //             onChange={(e: any) => setForm(f => ({ ...f, wardNo: e.target.value }))} />
// //         </div>

// //         {/* Date + AD/BS toggle */}
// //         <div className="grid gap-3" style={{ gridTemplateColumns: "2fr 1fr" }}>
// //           <DatePicker dateFormat="Y-m-d" onChange={(dates: Date[]) => {
// //             const d = dates?.[0];
...
// //             name="era"
// //             value={form.era}
// //             // CHANGE 1: use the wider param signature expected by Carbon
// //             onChange={(selection /* string|number|undefined */, _name, _evt) =>
...
// //             labelTitle=""
// //             labelDescription=""
// //             // CHANGE 2: required prop in your Carbon version
// //             filenameStatus="edit"
// //             // and use addedFiles from Carbon's event
...
//     evt.preventDefault();
//     if (validate()) setOpen(true);     // ✅ only open when valid
//   };
...
//         {/* Section 2: District / Address / Ward / Date + Era */}
...
//           }}>
//             <DatePickerInput id="date" labelText="Date" placeholder="yyyy-mm-dd"
//               invalid={!!errors.date} invalidText={errors.date} />
//           </DatePicker>
...
//       {/* Confirm Modal (only opens when valid) */}
//       <Modal
//         open={open}
//         modalHeading="Are you sure ?"
//         primaryButtonText={saving ? "Saving…" : "Confirm"}
...
export default function SubmitPage() {
  ...
  // --- simple required-field + date format validator (YYYY-MM-DD)
  const validate = () => {
    const e: Partial<Record<keyof FormState, string>> = {};
    if (!form.category) e.category = "Required";
    if (!form.district) e.district = "Required";
...
const confirmAndSave = async () => {
  ...
  try {
    ...
    // Redirect to the static page: /submit/success/id?ticket=<id>
  ...
  return (
    <main className="min-h-screen has-mobile-footer" style={{ padding: "1rem 1rem 6rem" }}>
      ...
      {/* noValidate suppresses native browser tooltip; we validate ourselves */}
      <form noValidate onSubmit={handleSubmit} className="grid gap-4" aria-describedby="submit-help">
        ...
        {/* Section 2: District / Address / Ward */}
        <div className="form-row grid gap-3" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
          <TextInput id="district" labelText="District" value={form.district}
            invalid={!!errors.district} invalidText={errors.district}
            onChange={(e: any) => setForm(f => ({ ...f, district: e.target.value }))} required />
        ...
        {/* Section 2b: Date + AD/BS toggle */}
        <div className="form-row grid gap-3" style={{ gridTemplateColumns: "2fr 1fr" }}>
          <DatePicker
            datePickerType="single"         // calendar UI + icon enabled
            dateFormat="Y-m-d"
            onChange={(dates: any) => {
          ...
          >
            <DatePickerInput
              ...
              /* no 'pattern' or 'iconDescription' props (not in your Carbon typings) */
            />
          </DatePicker>
